...
import { getHighscoresForBossPaginated } from "@/lib/db";
...
export default function HighscoresPage({ searchParams }: HighscoresPageProps) {
  // Default to first boss if not specified
  const selectedBoss = searchParams.boss ?? BOSSES[0];
  const page = parseInt(searchParams.page || "1", 10);
  const perPage = 25;

  // Get paginated data
  const { data: rankings, total } = getHighscoresForBossPaginated(
    selectedBoss,
    page,
    perPage
  );
        ...
        <div className="w-48 flex-shrink-0">
          <div className="rounded border border-neutral-800 bg-neutral-900">
            ...
            <nav className="divide-y divide-neutral-800">
              {BOSSES.map((boss) => {
                const isActive = boss.toLowerCase() === selectedBoss.toLowerCase();
                return (
                  <Link
                    key={boss}
                    href={`/highscores?boss=${encodeURIComponent(boss)}&page=1`}
        ...
        <div className="flex-1 space-y-4">
          ...
          {rankings.length === 0 ? (
            <div className="rounded border border-neutral-800 bg-neutral-900/50 px-6 py-8 text-center">
              <p className="text-neutral-400">
                No personal bests yet for {selectedBoss}.
              </p>
            </div>
          ) : (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              {page > 1 && (
                <Link
                  href={`/highscores?boss=${encodeURIComponent(selectedBoss)}&page=${page - 1}`}
              ...
              <div className="flex gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                  <Link
                    key={p}
                    href={`/highscores?boss=${encodeURIComponent(selectedBoss)}&page=${p}`}
